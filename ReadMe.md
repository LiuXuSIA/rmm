...
Assume that a robot has captured a data set $\boldsymbol X = \\{\boldsymbol x_i,t_i\\}_{i=1}^{L}$ by the end points of a laser ranger finder or depth cameras when moving in the environments, where $\boldsymbol x_i$ is a 2D location and $t_i$ is its elevation. Our idea for for the terrain modeling is to build a linear regression model between $ \\{\boldsymbol x_i\\}_{i=1}^{L}$ and $\\{t_i\\}_{i=1}^{L}$.
